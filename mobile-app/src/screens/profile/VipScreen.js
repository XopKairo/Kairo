        ...
        { 
          ...
          onPress: async () => {
            ...
            try {
              ...
              showAlert('Error', err.response?.data?.message || 'Failed to activate VIP', 'error');
        ...
        {packages.map((pkg) => (
          <TouchableOpacity key={pkg._id} style={styles.pkgCard} onPress={() => handleBuyVip(pkg)}>
             <View style={styles.pkgHeader}>
                <View><Text style={styles.pkgName}>{pkg.name}</Text><Text style={styles.pkgDuration}>{pkg.durationDays} Days Membership</Text></View>
             ...
             <View style={styles.featureList}>
                {(pkg.features || ["Priority Profile", "Exclusive Badge", "Daily Bonus"]).map((feat, i) => (
                   <View key={i} style={styles.featureItem}><CheckCircle2 color={COLORS.primary} size={16} /><Text style={styles.featureText}>{feat}</Text></View>
                ))}
             </View>
             <LinearGradient colors={[COLORS.primary, '#4C1D95']} style={styles.buyBtn}><Text style={styles.buyBtnText}>Activate Now</Text></LinearGradient>
          </TouchableOpacity>
        ))}

        <View style={styles.perksGrid}>
           <View style={styles.perkItem}><View style={[styles.perkIconBox, { backgroundColor: 'rgba(245, 158, 11, 0.1)' }]}><Crown color="#F59E0B" size={24} /></View><Text style={styles.perkTitle}>Elite Badge</Text></View>
           <View style={styles.perkItem}><View style={[styles.perkIconBox, { backgroundColor: 'rgba(16, 185, 129, 0.1)' }]}><Star color="#10B981" size={24} /></View><Text style={styles.perkTitle}>Fast Track</Text></View>
           <View style={styles.perkItem}><View style={[styles.perkIconBox, { backgroundColor: 'rgba(59, 130, 246, 0.1)' }]}><ShieldCheck color="#3B82F6" size={24} /></View><Text style={styles.perkTitle}>Incognito</Text></View>
        </View>
      </ScrollView>
