...
const fetchBonusSettings = async () => {
  const res = await axios.get(`${import.meta.env.VITE_API_URL}/admin/settings/bonus`);
  if (res.data) {
...
const saveBonusSettings = async () => {
  await axios.post(`${import.meta.env.VITE_API_URL}/admin/settings/bonus`, bonusSettings.value);
