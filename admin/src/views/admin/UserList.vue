...
const fetchUsers = async () => {
  try {
    const response = await axios.get('https://kairo-novh.onrender.com/api/users');
...
const banUser = async (id: string | number) => {
  if(confirm("Are you sure you want to ban this user?")) {
    try {
      await axios.post(`https://kairo-novh.onrender.com/api/users/${id}/ban`, { isBanned: true });
      fetchUsers();
    } catch (error) {
...
const updateCoins = async (id: number, currentCoins: number) => {
  ...
  if (newCoins !== null) {
    await axios.put(`https://kairo-novh.onrender.com/api/users/${id}`, { coins: parseInt(newCoins) });
    fetchUsers();
  }
};
