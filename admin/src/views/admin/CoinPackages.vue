...
const fetchPackages = async () => {
  const res = await axios.get(`${import.meta.env.VITE_API_URL}/economy/coins`);
  packages.value = res.data;
};

const savePackage = async () => {
  if (editedItem.value._id) {
    await axios.put(`${import.meta.env.VITE_API_URL}/economy/coins/${editedItem.value._id}`, editedItem.value);
  } else {
    await axios.post(`${import.meta.env.VITE_API_URL}/economy/coins`, editedItem.value);
  }
  dialog.value = false;
  fetchPackages();
};
...
const deletePackage = async (id: string) => {
  if(confirm("Delete this package?")) {
    await axios.delete(`${import.meta.env.VITE_API_URL}/economy/coins/${id}`);
    fetchPackages();
  }
};
