...
const fetchPendingHosts = async () => {
  try {
    const res = await axios.get(`${import.meta.env.VITE_API_URL}/hosts`);
...
const approveHost = async (id: string) => {
  try {
    ...
    await axios.post(`${import.meta.env.VITE_API_URL}/hosts/${id}/verify`, { isVerified: true });
    fetchPendingHosts();
  } catch (error) {
...
const rejectHost = async (id: string) => {
  if(confirm("Are you sure you want to delete/reject this host registration?")) {
    try {
      await axios.delete(`${import.meta.env.VITE_API_URL}/hosts/${id}`);
      fetchPendingHosts();
    } catch (error) {
