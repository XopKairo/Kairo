...
const saveGift = async () => {
  ...
  if (editedGift.value._id) {
    await axios.put(`${import.meta.env.VITE_API_URL}/economy/gifts/${editedGift.value._id}`, formData);
  } else {
    await axios.post(`${import.meta.env.VITE_API_URL}/economy/gifts`, formData);
  }
  dialog.value = false;
...
const deleteGift = async (id: string) => {
  if (confirm("Delete this gift?")) {
    await axios.delete(`${import.meta.env.VITE_API_URL}/economy/gifts/${id}`);
    fetchGifts();
  }
};
