...
const fetchSettings = async () => {
  try {
    const res = await axios.get(`${import.meta.env.VITE_API_URL}/admin/settings/app`);
    if (res.data) {
...
const saveSettings = async () => {
  try {
    await axios.post(`${import.meta.env.VITE_API_URL}/admin/settings/app`, settings.value);
