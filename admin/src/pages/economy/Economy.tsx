...
import { DollarSign, Plus, Edit, Trash2, X, Gift as GiftIcon, History, CreditCard } from 'lucide-react';
  ...
  const fetchData = async () => {
    try {
      const [coinsRes, transRes] = await Promise.all([
        ...
        apiClient.get('/admin/monitoring/transactions') // Placeholder for purchases
      ]);
    ...
    <div className="space-y-10">
      ...
      <div className="bg-white dark:bg-surface-900 rounded-[32px] p-8 shadow-sm">
         ...
         <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            {coins.map(pkg => (
              <div key={pkg._id} className="p-5 border rounded-3xl relative">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60">
          <div className="bg-white dark:bg-surface-900 w-full max-w-md rounded-[32px] p-8 shadow-2xl">
            ...
            <form onSubmit={handleSaveChanges} className="space-y-4">
              <input type="number" value={formData.coins} onChange={e => setFormData({...formData, coins: parseInt(e.target.value)})} className="w-full p-3 border rounded-xl" placeholder="Coins" />
              <input type="number" value={formData.bonus} onChange={e => setFormData({...formData, bonus: parseInt(e.target.value)})} className="w-full p-3 border rounded-xl" placeholder="Bonus" />
              <input type="number" value={formData.priceINR} onChange={e => setFormData({...formData, priceINR: parseInt(e.target.value)})} className="w-full p-3 border rounded-xl" placeholder="Price INR" />
