...
class WalletService {
  async processWithdrawal(data) {
    ...
    try {
      ...
      const existingRequest =
        await walletRepository.findPayoutByClientRequestId(
          clientRequestId,
          session,
        );
      if (existingRequest) {
      ...
      if (!userId || !amountCoins || !paymentDetails) {
        throw new Error(
          "User ID, amount in coins, and payment details are required",
        );
      }
      ...
      if (isAdminWithdrawal) {
        ...
        if (!admin || admin.totalRevenue < amountINR)
          throw new Error("Insufficient Admin revenue");
        ...
        return {
          success: true,
          message: "Admin withdrawal submitted",
          amountINR,
        };
      }
      ...
      const dailyPayouts = await walletRepository.findPayoutsByUserAndDate(
        userId,
        startOfDay,
        session,
      );
  ...
  async processEarnAd(userId, clientRequestId) {
    ...
    try {
      ...
      const existingTx = await mongoose
        .model("Transaction")
        .findOne({ orderId: clientRequestId })
        .session(session);
      if (existingTx) {
        throw new Error("DUPLICATE_REQUEST");
      }
      ...
      const now = new Date();
      const isSameDay =
        user.lastAdWatchedAt &&
        user.lastAdWatchedAt.toDateString() === now.toDateString();
      ...
      // ATOMIC UPDATE
      const balanceBefore = user.coins;
      const updatedUser = await userRepository.updateAdStats(
        userId,
        rewardAmount,
        isSameDay,
        limit,
