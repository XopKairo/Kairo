...
class GrowthService {
  ...
  async claimDailyReward(userId) {
    ...
    if (user.lastStreakClaimedAt && user.lastStreakClaimedAt.toDateString() === todayStr) {
    ...
    if (!user.lastStreakClaimedAt || user.lastStreakClaimedAt.toDateString() !== yesterday.toDateString()) {
    ...
    return { success: true, reward, currentStreak: user.currentStreak, newBalance: user.coins };
  }
  ...
  async getTopSpenders(period = 'daily') {
    ...
    return await mongoose.model('WalletLedger').aggregate([
  ...
  async getTopHosts(period = 'daily') {
    ...
    return await mongoose.model('Call').aggregate([
      ...
      { $lookup: { from: 'users', localField: '_id', foreignField: '_id', as: 'user' } },
      { $unwind: '$user' },
      { $project: { 'user.name': 1, 'user.profilePicture': 1, totalEarnings: 1 } }
    ]);
  }
}
