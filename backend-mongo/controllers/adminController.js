...
class AdminController {
  ...
  async adjustWallet(req, res) {
    ...
    try {
      ...
      res.json({ success: true, newBalance, message: `Successfully ${type.toLowerCase()}ed ${amount} coins` });
    } catch (error) {
  ...
  async forceEndCall(req, res) {
    ...
    try {
      ...
      if (!call) return res.status(404).json({ success: false, message: 'Active call not found' });
      ...
      if (req.io) {
        req.io.to(`user-${call.userId}`).emit('callTerminated', { reason: 'Force terminated by admin' });
        req.io.to(`host-${call.hostId}`).emit('callTerminated', { reason: 'Force terminated by admin' });
      }
  ...
  async addToBlacklist(req, res) {
    try {
      ...
      const item = await Blacklist.create({ value, type: type || 'IP', reason, adminId: req.admin?._id });
      ...
      } catch (logErr) { console.error('Blacklist logging failed:', logErr.message); }
