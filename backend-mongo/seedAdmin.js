...
const seedAdmin = async () => {
  ...
  if (!adminEmail || !adminPassword) {
    console.error(
      "❌ Error: ADMIN_EMAIL or ADMIN_PASSWORD is missing from environment variables.",
    );
    process.exit(1);
  }
  ...
  try {
    ...
    if (existingAdmin) {
      console.log(
        `ℹ️ An admin account already exists (Email: ${existingAdmin.email}).`,
      );
      console.log(
        "⏭️ Skipping creation to prevent overwriting existing credentials.",
      );
    } else {
